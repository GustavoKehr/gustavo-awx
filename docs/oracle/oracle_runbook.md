...
O playbook usa **dois diretórios** montados no EE container como source:

**`/opt/oracle/software/`** — installer zip, RPM, libnsl, OS Watcher:
```bash
ls -la /opt/oracle/software/
```
...
> **Como funciona:** O EE usa `ansible.posix.synchronize` (rsync push direto do controller para o target) — sem `delegate_to`. Os diretórios `/opt/oracle/software` e `/opt/patches` devem estar montados no EE container (configurado via operador no k3s).
...
EE precisa montar `/opt/oracle/software` e `/opt/patches` do host awxvm (configurado via operador no k3s). Verificar nos logs do job que ambos os paths existem.
...
| **Execution Environment** | `oracle-ee` (EE com `/opt/oracle/software` e `/opt/patches` montados) |
| **Playbook** | `playbooks/deploy_oracle.yml` |
| **Credentials** | `Machine: user_aap` |
...
ls -la /opt/oracle/software/LINUX.X64_193000_db_home.zip
ls -la /opt/oracle/software/oracle-database-preinstall-19c-1.0-1.el9.x86_64.rpm
ls -la /opt/oracle/software/libnsl_libs/
ls -la /opt/patches/p6880880/OPatch/
ls -la /opt/patches/p37641958/
ls -la /opt/patches/p38632161/
ls -la /opt/patches/p34672698/
ls -la /opt/oracle/software/oswbb840.tar
...
# OSW tar deve existir no controller (para Phase 0):
ls -la /opt/oracle/software/oswbb840.tar
```
