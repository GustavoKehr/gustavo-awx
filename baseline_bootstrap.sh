#!/bin/bash
# Script de Bootstrap para novas VMs

USER="ansible"
# A CHAVE PÚBLICA QUE VOCÊ ME PASSOU:
PUB_KEY="ssh-rsa  root@awxvm"

# 1. Cria o usuário e limpa vestígios de senha 
useradd $USER 2>/dev/null || echo "Usuário já existe"
echo "$USER:1" | chpasswd

# 2. Configura o Sudoers para não travar o Ansible
echo "$USER ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/$USER
chmod 440 /etc/sudoers.d/$USER

# 3. CRIA A PASTA E A "FECHADURA" (AUTHORIZED_KEYS)
mkdir -p /home/$USER/.ssh
echo "$PUB_KEY" > /home/$USER/.ssh/authorized_keys

# 4. PERMISSÕES CRÍTICAS
chown -R $USER:$USER /home/$USER/.ssh
chmod 700 /home/$USER/.ssh
chmod 600 /home/$USER/.ssh/authorized_keys

echo "Bootstrap completo. A chave pública foi autorizada!"

# 5. Update OS
dnf update -y